...
function getPortionsByCalories(cal) {
  if (cal <= 1300)      return { rice: 6,  vegetable: 6, fruit: 3, meat: 5, milk: 1, sugar: 3, fat: 3, salt: 1 };
  else if (cal <= 1500) return { rice: 7,  vegetable: 6, fruit: 3, meat: 5, milk: 1, sugar: 3, fat: 3, salt: 1 };
  else if (cal <= 1700) return { rice: 8,  vegetable: 6, fruit: 3, meat: 6, milk: 1, sugar: 3, fat: 5, salt: 1 };
  else if (cal <= 1900) return { rice: 9,  vegetable: 6, fruit: 4, meat: 7, milk: 1, sugar: 3, fat: 6, salt: 1 };
  else                  return { rice: 10, vegetable: 6, fruit: 4, meat: 8, milk: 1, sugar: 3, fat: 6, salt: 1 };
}

// ─── Portion calculators ─────────────────────────────────────────────
const calcRicePortions      = (logs) => logs.reduce((s, l) => { const c = Number(l.totalCarb || l.carb || l.carbs || 0); return c > 0 ? s + c / 18 : s + Number(l.servings || l.serving || l.amount || l.portions || 0); }, 0);
const calcVegetablePortions = (logs) => logs.reduce((s, l) => {
  // ถ้ามี items array (vegetableLogs format) — นับ portion ของแต่ละ item โดยตรง
  if (Array.isArray(l.items) && l.items.length > 0) {
    return s + l.items.reduce((sum, item) => sum + Number(item.portion || 0), 0);
  }
...
const calcFruitPortions     = (logs) => logs.reduce((s, l) => { const c = Number(l.totalCarb || l.carb || l.carbs || 0); return c > 0 ? s + c / 15 : s + Number(l.servings || l.serving || l.amount || 0); }, 0);
const calcMeatPortions      = (logs) => logs.reduce((s, l) => { const p = Number(l.totalProtein || l.protein || l.proteins || 0); return p > 0 ? s + p / 7 : s + Number(l.servings || l.serving || l.amount || 0); }, 0);
const calcDairyPortions     = (logs) => logs.reduce((s, l) => { const p = Number(l.totalProtein || l.protein || l.proteins || 0); return p > 0 ? s + p / 8 : s + Number(l.servings || l.serving || l.amount || 0); }, 0);
const calcFatPortions       = (logs) => logs.reduce((s, l) => { const f = Number(l.totalFat || l.fat || 0); return f > 0 ? s + f / 5 : s + Number(l.servings || l.serving || l.amount || 0); }, 0);
const calcSaltPortions      = (logs) => logs.reduce((s, l) => s + Number(l.servings || l.serving || l.amount || 0), 0);

// ─── Today's date in Thai ────────────────────────────────────────────
function getTodayThai() {
  const now = new Date();
  return now.toLocaleDateString("th-TH", { year: "numeric", month: "short", day: "numeric" });
}
...
export default function MealPage() {
  ...
  const [consumed, setConsumed] = useState({ rice: 0, vegetable: 0, fruit: 0, meat: 0, milk: 0, sugar: 0, fat: 0, salt: 0 });
  ...
  useEffect(() => {
    ...
    if (userDataStr) {
      ...
      setUserTarget({ calories: Number(ud.calories) || 2000, protein: Number(ud.protein) || 100, carbs: Number(ud.carbs) || 250, fat: Number(ud.fat) || 67 });
    }

    const getLogs  = (key) => JSON.parse(localStorage.getItem(`${key}_${currentUser}`) || "[]");
    ...
    const allLogs = [...todayRice, ...todayFruit, ...todayDairy, ...todayMeat, ...todayVegetable, ...todayFat, ...todayDessert, ...todayDish, ...todayCondiment]
      .map((log) => {
        ...
        const kcal    = Number(log.totalCalories || log.calories || 0) || (carb * 4 + protein * 4 + fat * 9);
        return { ...log, kcal, carb, protein, fat };
      });

    setSummary(allLogs.reduce((acc, l) => ({ kcal: acc.kcal + l.kcal, protein: acc.protein + l.protein, carb: acc.carb + l.carb, fat: acc.fat + l.fat }), { kcal: 0, protein: 0, carb: 0, fat: 0 }));

    const bd = { breakfast: { kcal: 0, protein: 0, carb: 0, fat: 0, items: [] }, lunch: { kcal: 0, protein: 0, carb: 0, fat: 0, items: [] }, dinner: { kcal: 0, protein: 0, carb: 0, fat: 0, items: [] }, snack: { kcal: 0, protein: 0, carb: 0, fat: 0, items: [] } };
    allLogs.forEach((log) => {
      const m = log.mealId || "snack";
      if (bd[m]) { bd[m].kcal += log.kcal; bd[m].protein += log.protein; bd[m].carb += log.carb; bd[m].fat += log.fat; bd[m].items.push(log); }
    });
    setMealBreakdown(bd);
  }, []);
  ...
  const foodCategories = [
    { id: "rice",      name: "ข้าว - แป้ง",  icon: "🍚", unit: "ทัพพี",       target: portions.rice,      consumed: consumed.rice,      color: "#2CB9A8" },
    { id: "vegetable", name: "ผัก",         icon: "🥬", unit: "ทัพพี",       target: portions.vegetable, consumed: consumed.vegetable, color: "#2CB9A8" },
    { id: "fruit",     name: "ผลไม้",       icon: "🍎", unit: "ส่วน",        target: portions.fruit,     consumed: consumed.fruit,     color: "#2CB9A8" },
    { id: "meat",      name: "เนื้อสัตว์",  icon: "🥩", unit: "ช้อนกินข้าว", target: portions.meat,      consumed: consumed.meat,      color: "#2CB9A8" },
    { id: "dairy",     name: "นม",          icon: "🥛", unit: "แก้ว",        target: portions.milk,      consumed: consumed.milk,      color: "#2CB9A8" },
    { id: "sugar",     name: "น้ำตาล",      icon: "🍬", unit: "ช้อนชา",      target: portions.sugar,     consumed: consumed.sugar,     color: "#2CB9A8", isLimit: true },
    { id: "fat",       name: "น้ำมัน",      icon: "🧈", unit: "ช้อนชา",      target: portions.fat,       consumed: consumed.fat,       color: "#2CB9A8", isLimit: true },
    { id: "salt",      name: "เกลือ",       icon: "🧂", unit: "ช้อนชา",      target: portions.salt,      consumed: consumed.salt,      color: "#2CB9A8", isLimit: true },
  ];
  ...
  return (
    <div style={s.page}>
      <div style={s.wrapper}>
        ...
        <div style={s.topBar}>
          <div>
            <div style={s.brand}><span style={s.brandBold}>Smart</span><span style={s.brandAccent}>DM</span></div>
        ...
        <div style={s.calorieCard}>
          ...
          <div style={s.calorieRow}>
            <span style={s.calorieNum}>{Math.round(summary.kcal).toLocaleString()}</span>
            <span style={s.calorieOf}> / {userTarget.calories.toLocaleString()} kcal</span>
            <span style={{ ...s.caloriePct, color: isKcalOver ? "#EF4444" : "#2CB9A8" }}>
              {Math.round(kcalPct)}%
            </span>
          </div>
        ...
        <div style={s.sectionCard}>
          ...
          <div style={s.mealRow}>
            {meals.map((meal) => {
              ...
              return (
                <div key={meal.id} style={s.mealItem} onClick={() => navigate(`/food-log?mealId=${meal.id}&mealName=${encodeURIComponent(meal.name)}`)}>
            ...
            {/* snack as "+ เพิ่ม" */}
            <div style={s.mealItem} onClick={() => navigate(`/food-log?mealId=snack&mealName=${encodeURIComponent("มื้อว่าง")}`)}>
              <span style={s.mealIcon}>🍪</span>
              <div style={s.mealName}>มื้อว่าง</div>
              <div style={s.mealSub}>{mealBreakdown.snack?.kcal > 0 ? `${Math.round(mealBreakdown.snack.kcal)} kcal` : "+ เพิ่ม"}</div>
            </div>
          </div>
        </div>
        ...
        <div style={s.insightCard}>
          ...
          <div style={s.insightGrid}>
            <InsightItem status={getStatus(summary.carb, userTarget.carbs * 0.5, userTarget.carbs)}   label="คาร์บ"    good="เหมาะสม" low="ยังน้อย" bad="เกินเกณฑ์" />
            <InsightItem status={getStatus(consumed.fruit, portions.fruit * 0.7, portions.fruit * 1.5)} label="ผลไม้"   good="เพียงพอ" low="ยังน้อย" bad="มากเกิน" />
            <InsightItem status={getStatus(summary.protein, userTarget.protein * 0.8, userTarget.protein * 1.3)} label="โปรตีน" good="เพียงพอ" low="ยังน้อย" bad="เกินเกณฑ์" />
            <InsightItem status={getStatus(consumed.vegetable, portions.vegetable * 0.7, portions.vegetable * 1.5)} label="ผัก"  good="เพียงพอ" low="ยังน้อย" bad="มากเกิน" />
          </div>
        </div>
...
const s = {
  ...
  wrapper: {
    maxWidth: 480, width: "100%", minHeight: "100vh",
  ...
  foodCard: { background: "white", borderRadius: 16, padding: "10px 14px", display: "flex", flexDirection: "column", boxShadow: "0 2px 10px rgba(0,0,0,0.05)" },
  foodCardLeft: { flex: 1 },
